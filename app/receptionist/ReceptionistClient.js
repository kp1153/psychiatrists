  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const prescRes = await fetch('/api/prescriptions', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ patient_id: patient.id, complaints: complaints.trim(), status: initialStatus }),
      });
      if (!prescRes.ok) {
            ...
            {success && (
              <div className="bg-green-50 border border-green-200 rounded-xl p-4 text-sm text-green-800">
                ...
                <p className="text-xs text-gray-500 mt-1">
                  {success.hasPsychologist ? '→ Sent to Psychologist queue' : '→ Sent to Doctor queue'}
                </p>
              </div>
            )}
            <button
