...
export default function SettingsPage() {
  ...
  async function handleSave() {
    ...
    await fetch('/api/settings', {
      ...
      body: JSON.stringify({ pin_receptionist: pinR, pin_pharmacy: pinP, name: clinicName }),
    });
    setSaving(false);
    setSaved(true);
  ...
  if (!clinic) return <p className="text-center mt-20 text-gray-400">Loading...</p>;

  return (
    <main className="min-h-screen bg-gray-50 p-4 pb-20">
      <div className="max-w-md mx-auto">
        <button onClick={() => router.back()} className="text-indigo-700 text-sm mb-4 mt-2">← Back</button>
        ...
        <div className="bg-white rounded-2xl shadow p-5 flex flex-col gap-5">
          ...
          <div>
            <label className="block text-sm font-semibold text-gray-600 mb-1">Clinic Name</label>
            <input
              type="text"
              value={clinicName}
          ...
          <div>
            <label className="block text-sm font-semibold text-gray-600 mb-1">Receptionist PIN</label>
            <input
              type="password"
              value={pinR}
          ...
          <div>
            <label className="block text-sm font-semibold text-gray-600 mb-1">Pharmacy PIN</label>
            <input
              type="password"
              value={pinP}
          ...
          {saved && <p className="text-green-600 text-sm text-center">✓ Saved successfully</p>}
