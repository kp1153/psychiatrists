...
export default function DoctorPrescriptionPage() {
  ...
  const [medicines, setMedicines] = useState([
    { name: "", dose: "", timing: [], duration: "7 days" },
  ]);
  ...
  useEffect(() => {
    async function load() {
      ...
      const pRes = await fetch(`/api/patients?phone=`);
      // get patient via prescriptions join
  ...
  const pickerList = selectedCondition
    ? MEDICINES_BY_CONDITION[selectedCondition] || []
    : [];
  ...
  return (
    <>
      ...
      <main
        className="min-h-screen bg-emerald-50 p-4 pb-20 no-print"
        ref={printRef}
      >
        <div className="max-w-lg mx-auto">
          <button
            onClick={() => router.back()}
            className="no-print text-emerald-700 text-sm mb-4 mt-2 flex items-center gap-1"
          >
            ← Back to Queue
          </button>
          ...
          {/* Quick picker from conditions */}
          <div className="bg-white rounded-2xl shadow p-4 mb-4">
            <div className="flex justify-between items-center mb-3">
              <label className="font-semibold text-gray-700">
                Quick Add by Condition
              </label>
              <button
                type="button"
            ...
            {showPicker && (
              <>
                <select
                  ...
                  className="w-full border border-gray-300 rounded-xl px-3 py-2 text-sm mb-3 focus:outline-none focus:ring-2 focus:ring-indigo-400"
                >
                ...
                {selectedCondition && pickerList.length > 0 && (
                  <div className="flex flex-col gap-2 max-h-80 overflow-y-auto">
                    {pickerList.map((med, idx) => (
                      <button
                        key={`${selectedCondition}-${idx}`}
                        type="button"
                        onClick={() => addFromPicker(med)}
                        className="text-left border border-gray-200 rounded-lg px-3 py-2 hover:bg-indigo-50 active:scale-[0.98] transition"
                      >
                ...
                {selectedCondition && pickerList.length === 0 && (
                  <p className="text-xs text-gray-400 text-center py-2">
                    No medicines listed for this condition.
                  </p>
                )}
              </>
            )}
                ...
                >
                  <div className="flex gap-2">
                    ...
                    <input
                      ...
                      placeholder="Dose"
                      className="w-20 border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-emerald-400"
                    />
                  </div>
      ...
      <div className="hidden print:block p-8">
        ...
        {prescription.complaints && (
          <p className="text-sm mb-3">
            <strong>Complaints:</strong> {prescription.complaints}
          </p>
        )}
        {tests && (
          <p className="text-sm mb-3">
