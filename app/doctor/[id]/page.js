...
export default function DoctorPrescriptionPage() {
  ...
  const addedNames = useMemo(() => {
    ...
    for (const m of medicines) {
      if (m.name) s.add(m.name.split(" (")[0].toLowerCase());
    }
    return s;
  }, [medicines]);
  ...
  function addWithDose(med, chosenDose) {
    ...
    setMedicines((prev) => {
      ...
      const already = prev.some((m) => m.name.toLowerCase().startsWith(med.name.toLowerCase()));
  ...
  function doseOptionsFor(medName) {
    const base = medName.split(" (")[0].trim().toLowerCase();
    for (const cond of CONDITIONS) {
      const list = MEDICINES_BY_CONDITION[cond] || [];
